...
function ReviewCard({ review, index }: ReviewCardProps): JSX.Element {
    ...
    return (
        <div
            className={`mx-4 min-w-[400px] max-w-[400px] ${isLight ? 'bg-[#fdf9f0]' : 'bg-[#1a1a1a]'} border ${isLight ? 'border-[#e5e0d5]' : 'border-[#333]'} rounded-lg p-6 flex-shrink-0`}
        >
            ...
            <p
                className={`${isLight ? 'text-gray-700' : 'text-gray-300'} text-sm mb-4 leading-relaxed text-left`}
            >
                {review.review}
            </p>
            <div className={`border-t ${isLight ? 'border-[#e5e0d5]' : 'border-[#333]'} pt-4`}>
                <p
                    className={`${isLight ? 'text-gray-900' : 'text-white'} font-semibold text-left`}
                >
                    {review.name}
                </p>
                <p className={`${isLight ? 'text-gray-600' : 'text-gray-500'} text-sm text-left`}>
                    {review.role}
                </p>
            </div>
        </div>
...
function MarqueeRow({ reviews, duration }: MarqueeRowProps): JSX.Element {
    return (
        <div className="relative overflow-hidden py-4">
            ...
            >
                {[...reviews, ...reviews, ...reviews].map((review: Review, idx: number) => (
    ...
    return (
        <div className="min-h-screen bg-primary z-20">
            ...
            <div className="mt-16 space-y-6">
                <MarqueeRow reviews={firstRow} duration={20} />
                <MarqueeRow reviews={secondRow} duration={25} />
            </div>
        </div>
    );
}
